...
load cam2_1.mat;
load cam3_1.mat;
vidFrames2_1 = vidFrames2_1(:,:,:,10:end);
...
% coords1 = zeros([2 num_frames-1]);
% frame = im2double(rgb2gray(vidFrames1_1(:,:,:,1)));
% for j=2:num_frames
%     X=rgb2gray(vidFrames1_1(:,:,:,j));
%     X=im2double(X);
%     diff = X - frame;
%     [M,I] = max(abs(diff(:)));
%     [y,x] = ind2sub([height1 width1],I);
%     coords1(:,j-1) = [x y];
%     frame = X;
% end
% 
% plot(1:num_frames-1,coords1(1,:))
